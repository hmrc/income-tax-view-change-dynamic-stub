...
class MicroserviceAuthConnector @Inject()(servicesConfig: ServicesConfig,
  ...
  def login(credId:              String = "", affinityGroup:String ="", credentialRole:      String = "",
            confidenceLevel:     String ="2", credentialStrength:  String = "2",
            enrolments:          Seq[EnrolmentData] = Nil,
            ...
            oauthTokens:         Option[OAuthTokens] = None, additionalInfo:      Option[AdditionalInfo] = None
    ...
    http.POST[JsValue, HttpResponse](s"$serviceUrl/government-gateway/session/login", payload) flatMap {
      case response @ HttpResponse(201, _, _) =>
        (
          ...
          case (Some(token), Some(sessionUri), Some(receivedGatewayToken)) => Future.successful((AuthExchange(token, sessionUri), GovernmentGatewayToken(receivedGatewayToken)))
      ...
      case response @ HttpResponse(429, _, _) =>
        Future.failed(new TooManyRequestException(s"response from/government-gateway/session/login was ${response.status}. Body ${response.body}"))
      case response =>
        Future.failed(new RuntimeException(s"response from /government-gateway/session/login was ${response.status}. Body ${response.body}"))
    }
  }
